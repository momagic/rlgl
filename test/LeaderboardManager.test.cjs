  ...
  WhackLight: 2
    ...
    describe("Seeding and Admin Functions", function () {
        ...
        it("Should reject seeding from non-owner", async function () {
            ...
            ).to.be.revertedWithCustomError(leaderboardManager, "OwnableUnauthorizedAccount");
        });
    ...
    describe("Upgradeability", function () {
        it("Should be upgradeable by owner", async function () {
            ...
            await expect(
                upgrades.upgradeProxy(leaderboardManager, LeaderboardManagerV2)
            ).to.not.be.reverted;
        });
        ...
        it("Should reject upgrade from non-owner", async function () {
            ...
            await expect(
                leaderboardManager.connect(unauthorizedUser).upgradeToAndCall(await newImplementation.getAddress(), "0x")
            ).to.be.revertedWithCustomError(leaderboardManager, "OwnableUnauthorizedAccount");
        });
    });
    ...
    describe("Gas Optimization Tests", function () {
        ...
        it("Should enforce batch size limits", async function () {
            ...
            const tooManyPlayers = new Array(21).fill(ethers.ZeroAddress);
