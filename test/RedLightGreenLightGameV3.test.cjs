...
describe("RedLightGreenLightGameV3", function () {
    ...
    describe("Deployment", function () {
        ...
        it("Should have correct verification multipliers", async function () {
            ...
            expect(Number(multipliers.currentOrbPlusMultiplier)).to.equal(140);
            expect(Number(multipliers.currentOrbMultiplier)).to.equal(125);
            expect(Number(multipliers.currentSecureDocumentMultiplier)).to.equal(115);
            expect(Number(multipliers.currentDocumentMultiplier)).to.equal(100);
        });
    ...
    describe("Game Mechanics", function () {
        ...
        it("Should not allow unverified users to start game", async function () {
            ...
            try {
                await v3Game.connect(unverifiedPlayer).startGame();
                expect.fail("Should have reverted");
            } catch (error) {
                expect(error.message).to.include("Document verification or higher required");
            }
        });
        ...
        it("Should not allow starting game without turns", async function () {
            ...
            try {
                ...
                expect(error.message).to.include("No turns available");
            }
        });
    ...
    describe("Verification System", function () {
        it("Should set user verification correctly", async function () {
            ...
            expect(Number(status.verificationLevel)).to.equal(VERIFICATION_LEVELS.OrbPlus);
            expect(status.isVerified).to.be.true;
            expect(Number(status.multiplier)).to.equal(140);
        });
        ...
        it("Should apply correct multipliers for each verification level", async function () {
            ...
            for (const { level, expected } of levels) {
                ...
                const status = await v3Game.getUserVerificationStatus(player1.address);
                expect(Number(status.multiplier)).to.equal(expected);
            }
        });
